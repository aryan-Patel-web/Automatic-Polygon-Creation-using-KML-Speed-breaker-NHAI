...
class PolygonSpec:
    ...
    strip_length_override_m: float = -1.0 # -1 = auto (= lane width); >0 = manual metres
    # Road layout
    num_lanes: int               = 2
...
def generate_polygon_for_marker(
    ...
    # Strip length: manual override or auto (= lane width)
    if spec.strip_length_override_m > 0:
        strip_len = spec.strip_length_override_m
    else:
        strip_len = lane_w
    ...
    for lane_idx in range(nl):
        ...
        if nl == 1:
            # Single lane: strip centred on road, length = strip_len
            pa, pb = -strip_len / 2.0, strip_len / 2.0

        elif lane_idx % 2 == 0:
            # Left side lanes (positive perpendicular)
            tier = lane_idx // 2
            inner = sep_h + tier * lane_w
            # Centre the strip within the lane
            lane_centre = inner + lane_w / 2.0
